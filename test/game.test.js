...
test("5 人确认庄前按本人等级理牌，确认庄后统一按庄家等级理牌", () => {
  ...
  assert.equal(room.seats[1].hand[0].rank, "A", "庄家确认前，其他玩家仍应按自己的 A 理牌");
  for (let i = 1; i < 5; i += 1) passBid(room, `p${i}`);
  assert.equal(room.phase, "burying");
  assert.equal(room.seats[1].hand[0].rank, "4", "庄家确认后，其他玩家才统一按庄家打 4 理牌");
});
...
test("5/6 人无主：小王加同花色常主可组成对子/三条拖拉机，两组常主不算", () => {
  for (const seatCount of [5, 6]) {
    const room = createRoom(`NTTRACTOR${seatCount}`, { seatCount });
    room.levelRank = "7";
    room.noTrump = true;
    room.trumpSuit = null;
    const deck = createDeck(3);
    const smallJokers = deck.filter((card) => card.rank === "smallJoker").slice(0, 2);
    const smallJokers3 = deck.filter((card) => card.rank === "smallJoker").slice(0, 3);
    const hearts7 = deck.filter((card) => card.rank === "7" && card.suit === "hearts").slice(0, 2);
    const hearts7_3 = deck.filter((card) => card.rank === "7" && card.suit === "hearts").slice(0, 3);
    const clubs7 = deck.filter((card) => card.rank === "7" && card.suit === "clubs").slice(0, 2);
    const clubs7_3 = deck.filter((card) => card.rank === "7" && card.suit === "clubs").slice(0, 3);

    assert.equal(analyzeShape([...smallJokers, ...hearts7], room).type, "tractor", `${seatCount} 人无主小王对子+常主对子应是拖拉机`);
    assert.equal(analyzeShape([...hearts7, ...clubs7], room).type, "throw", `${seatCount} 人无主两对常主不构成拖拉机`);
    assert.deepEqual(
      analyzeShape([...smallJokers3, ...hearts7_3], room),
      { type: "tractor", unit: 3, count: 2, value: 990 },
      `${seatCount} 人无主 3 小王+3 同花色常主应是三条拖拉机`
    );
    assert.equal(analyzeShape([...hearts7_3, ...clubs7_3], room).type, "throw", `${seatCount} 人无主两组三条常主不构成拖拉机`);
  }
});

test("5/6 人无主三条拖拉机：跟牌需优先满足两组三条，否则可降级三对子", () => {
  for (const seatCount of [5, 6]) {
    const room = createRoom(`NTTRIFOLLOW${seatCount}`, { seatCount });
    room.levelRank = "7";
    room.noTrump = true;
    room.trumpSuit = null;
    const deck = createDeck(3);
    const cards = (rank, suit, count) => deck.filter((card) => card.rank === rank && card.suit === suit).slice(0, count);
    const lead = [...cards("smallJoker", "joker", 3), ...cards("7", "hearts", 3)];
    const follower = room.seats[1];

    follower.hand = [...cards("7", "clubs", 3), ...cards("7", "diamonds", 3)];
    assert.equal(validatePlay(room, follower, follower.hand, lead).ok, true, `${seatCount} 人无主三条拖拉机：有两组三条时可按两组三条跟`);

    follower.hand = [...cards("7", "clubs", 2), ...cards("7", "diamonds", 2), ...cards("7", "spades", 2)];
    assert.equal(validatePlay(room, follower, follower.hand, lead).ok, true, `${seatCount} 人无主三条拖拉机：无三条时三对子可降级跟`);
  }
});

test("升级分档", () => {
  assert.deepEqual(upgradeResult(40), { side: "dealer", steps: 3, label: "庄家队升 3 级" });
  assert.deepEqual(upgradeResult(41), { side: "dealer", steps: 2, label: "庄家队升 2 级" });
...
test("5 人抢庄：3 张王可以盖过 3 张常主并夺庄", () => {
  const room = createRoom("FIVEJOKERCOVER");
  const deck = createDeck();
  for (let i = 0; i < 5; i += 1) {
    room.seats[i].playerId = `p${i}`;
    room.seats[i].nickname = `P${i}`;
  ...
  const hearts7 = deck.filter((card) => card.rank === "7" && card.suit === "hearts").slice(0, 3);
  const jokers = deck.filter((card) => card.suit === "joker").slice(0, 3);
  room.seats[0].hand.push(...hearts7);
  room.seats[1].hand.push(...jokers);

  makeBid(room, "p0", hearts7.map((card) => card.id));
  makeBid(room, "p1", jokers.map((card) => card.id));

  assert.equal(room.currentBid.seat, 1);
  assert.equal(room.dealerSeat, 1);
  assert.equal(room.noTrump, true);
  assert.ok(room.currentBid.strength > 3);
});
...
test("发牌阶段有人亮主后，未确认庄前继续按本人等级理牌", () => {
  ...
  room.seats[2].hand = [card("A", "spades", 2), card("2", "hearts", 1)];
  ...
  assert.equal(room.seats[2].hand[0].rank, "A", "庄家未确认前，亮主花色不能提前变成别人手里的主花色");
});

test("5 人抢庄等待确认时不按已亮庄家重排手牌", () => {
  const room = createRoom("AUCTIONSORT");
  const deck = createDeck();
  const card = (rank, suit, copy = 1) => deck.find((c) => c.rank === rank && c.suit === suit && c.copy === copy);
  for (let i = 0; i < 5; i += 1) {
    const s = room.seats[i];
  ...
  room.phase = "auctionReady";
  room.dealing = false;
  room.firstLevel = "K";
  room.seats[0].hand = [card("4", "hearts", 1)];
  room.seats[1].hand = [card("A", "spades", 1), card("2", "hearts", 1)];

  makeBid(room, "p0", [card("4", "hearts", 1).id]);
  assert.equal(room.seats[1].hand[0].rank, "A", "确认庄前不应把红桃 2 提前排进主牌区");

  for (let i = 1; i < 5; i += 1) passBid(room, `p${i}`);
  assert.equal(room.phase, "burying");
  assert.equal(room.seats[1].hand[0].rank, "2", "确认庄后才按庄家花色重排手牌");
});
...
test("图片头像坐下和接管 AI 时保留完整路径", () => {
  const avatar = "img:avatars/025Pikachu_OS_anime_4.webp";
  const unicodeAvatar = "img:avatars/赌神.jpeg";
  const room = createRoom("AVATAR");
  sit(room, "p1", 0, "P1", avatar);
  assert.equal(room.seats[0].avatar, avatar);
  sit(room, "p3", 2, "P3", unicodeAvatar);
  assert.equal(room.seats[2].avatar, unicodeAvatar);

  addAiPlayer(room, 1, "weak");
  takeoverAiSeat(room, "p2", 1, "P2", avatar);
  assert.equal(room.seats[1].avatar, avatar);
});
...
for (const seatCount of [5, 6]) {
  ...
  test(`${seatCount} 人出牌一致性：三条锁定后两张同点可按两张单牌出`, () => {
    ...
    assert.equal(validatePlay(room, follower, locked8, null).ok, true);
  });
...
test("5/6 人甩牌首家两对时，后手三条加单张不能压过两对", () => {
  for (const seatCount of [5, 6]) {
    const room = createRoom(`THROWPAIRTRIPLE${seatCount}`, { seatCount });
    room.levelRank = "8"; room.trumpSuit = "spades";
    const deck = createDeck();
    const cardsOf = (rank, suit, n = 1) => deck.filter((c) => c.rank === rank && c.suit === suit).slice(0, n);
    const lead = [...cardsOf("7", "diamonds", 2), ...cardsOf("3", "diamonds", 2)];
    const followSameSuit = [...cardsOf("Q", "diamonds", 3), cardsOf("6", "diamonds", 1)[0]];
    const cutWithTrumpTriple = [...cardsOf("Q", "spades", 3), cardsOf("6", "spades", 1)[0]];
    const follower = room.seats[1];
    follower.hand = [...followSameSuit];
    follower.lockedTriples = [];

    assert.equal(analyzeShape(lead, room).type, "throw");
    assert.equal(validatePlay(room, follower, followSameSuit, lead).ok, true);

    room.currentTrick = [
      { seat: 0, cards: lead, shape: analyzeShape(lead, room), points: 0 },
      { seat: 1, cards: followSameSuit, shape: analyzeShape(followSameSuit, room), points: 0 }
    ];
    assert.equal(determineTrickWinner(room, room.currentTrick), 0, `${seatCount} 人同花色三条+单张不能压两对`);

    room.currentTrick = [
      { seat: 0, cards: lead, shape: analyzeShape(lead, room), points: 0 },
      { seat: 1, cards: cutWithTrumpTriple, shape: analyzeShape(cutWithTrumpTriple, room), points: 0 }
    ];
    assert.equal(determineTrickWinner(room, room.currentTrick), 0, `${seatCount} 人主牌三条+单张不能按两对结构毙牌`);
  }
});

test("5/6 人甩牌两对时，锁定主三条拆两张不能当对子毙牌", () => {
  for (const seatCount of [5, 6]) {
    const room = createRoom(`THROWPAIRLOCKEDTRIPLE${seatCount}`, { seatCount });
    room.levelRank = "8"; room.trumpSuit = "spades";
    const deck = createDeck();
    const cardsOf = (rank, suit, n = 1) => deck.filter((c) => c.rank === rank && c.suit === suit).slice(0, n);
    const lead = [...cardsOf("7", "diamonds", 2), ...cardsOf("3", "diamonds", 2)];
    const lockedPair = cardsOf("Q", "spades", 2);
    const naturalPair = cardsOf("6", "spades", 2);
    const cut = [...lockedPair, ...naturalPair];
    room.seats[1].lockedTriples = ["Q|spades"];
    room.currentTrick = [
      { seat: 0, cards: lead, shape: analyzeShape(lead, room), points: 0 },
      { seat: 1, cards: cut, shape: analyzeShape(cut, room), points: 0 }
    ];

    assert.equal(determineTrickWinner(room, room.currentTrick), 0, `${seatCount} 人锁定三条拆两张只能算单张，不能补成两对杀`);
  }
});
...
test("需要对子但只有三条时，不拆出单张会锁定三条，之后不能拆成对子领出", () => {
  ...
  assert.equal(validatePlay(room, follower, triple.slice(0, 2), null).ok, true, "锁定后三条可以拆出两张，但只能视作两张单牌");
  room.currentTrick = [];
  room.currentLeader = 1;
  room.turnSeat = 1;
  playCards(room, "p1", triple.slice(0, 2).map((c) => c.id));
  const lockedPlay = room.currentTrick.at(-1);
  assert.equal(lockedPlay.shape.type, "throw", "锁定三条拆出两张同点应按两张单牌/甩牌结构记录，不算对子");
  assert.equal(lockedPlay.shapeLabel, "两张单9");
...
test("6 人首轮抢庄：3 张王可以反常主并坐庄", () => {
  const room = createRoom("FIXED6OPENJOKER", { seatCount: 6 });
  ...
  const club2s = deck.filter((card) => card.rank === "2" && card.suit === "clubs").slice(0, 2);
  const jokers = deck.filter((card) => card.suit === "joker").slice(0, 3);
  room.seats[0].hand.push(...club2s);
  room.seats[1].hand.push(...jokers);

  callSixTrump(room, "p0", club2s.map((card) => card.id));
  callSixTrump(room, "p1", jokers.map((card) => card.id));
...
test("6 人发牌阶段：3 张王可以反主为无主", () => {
  const room = createRoom("FIXED6DEALJOKERCOVER", { seatCount: 6 });
  for (let i = 0; i < 6; i += 1) { const s = room.seats[i]; s.playerId = `p${i}`; s.nickname = `P${i}`; }
  room.phase = "dealing";
  room.dealing = true;
  room.round = 2;
  ...
  const club5s = deck.filter((card) => card.rank === "5" && card.suit === "clubs").slice(0, 2);
  const jokers = deck.filter((card) => card.suit === "joker").slice(0, 3);
  room.seats[0].hand.push(...club5s);
  room.seats[1].hand.push(...jokers);

  makeBid(room, "p0", club5s.map((card) => card.id));
  makeBid(room, "p1", jokers.map((card) => card.id));
...
test("6 人结算分线：每 60 分一档，闲家升分不封顶", () => {
  assert.deepEqual(upgradeResultSix(0), { side: "dealer", steps: 3, label: "庄家队升 3 级" });
  assert.deepEqual(upgradeResultSix(1), { side: "dealer", steps: 2, label: "庄家队升 2 级" });
  assert.deepEqual(upgradeResultSix(59), { side: "dealer", steps: 2, label: "庄家队升 2 级" });
  assert.deepEqual(upgradeResultSix(60), { side: "dealer", steps: 1, label: "庄家队升 1 级" });
  assert.deepEqual(upgradeResultSix(119), { side: "dealer", steps: 1, label: "庄家队升 1 级" });
  assert.deepEqual(upgradeResultSix(120), { side: "attackers", steps: 0, label: "闲家队上台，不升级" });
  assert.deepEqual(upgradeResultSix(179), { side: "attackers", steps: 0, label: "闲家队上台，不升级" });
  assert.deepEqual(upgradeResultSix(180), { side: "attackers", steps: 1, label: "闲家队上台，升 1 级" });
  assert.deepEqual(upgradeResultSix(240), { side: "attackers", steps: 2, label: "闲家队上台，升 2 级" });
  assert.deepEqual(upgradeResultSix(300), { side: "attackers", steps: 3, label: "闲家队上台，升 3 级" });
  assert.deepEqual(upgradeResultSix(360), { side: "attackers", steps: 4, label: "闲家队上台，升 4 级" });
});
...
test("6 人结算轮庄：120-179 闲家上台不升级，下家坐庄", () => {
